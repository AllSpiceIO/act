...
func (cr *containerReference) connectToNetwork(name string, aliases []string) common.Executor {
	return func(ctx context.Context) error {
		_, err := cr.cli.NetworkConnect(ctx, name, client.NetworkConnectOptions{
			...
			EndpointConfig: &networktypes.EndpointSettings{Aliases: aliases},
		})
		return err
	}
}
...
func (cr *containerReference) waitForCommand(ctx context.Context, isTerminal bool, resp client.ExecAttachResult, _ client.ExecCreateResult, _ string, _ string) error {
	logger := common.Logger(ctx)
